...
@section('content')
@php($bgLoginUrl = asset('images/background-login.png'))
<div class="fixed inset-0 z-50 flex flex-col md:flex-row h-dvh md:min-h-screen w-full bg-white overflow-y-auto">

    <div class="w-full md:w-1/2 h-40 sm:h-64 md:h-auto bg-cover bg-center relative flex flex-col justify-end p-4 sm:p-8 md:p-12 lg:p-16 shrink-0"
        style="background-image: url('{{ $bgLoginUrl }}');">
        ...
        <div class="relative z-10 text-white">
            <h1 class="text-base sm:text-2xl md:text-3xl lg:text-4xl font-bold leading-tight mb-1 md:mb-4">
                Berikan kenyamanan<br class="hidden sm:inline">
                terbaik untuk sahabat setia<br class="hidden sm:inline">
                Anda.
            </h1>
            <p class="hidden sm:block text-xs sm:text-sm md:text-base lg:text-lg text-gray-100 font-light max-w-md">
                Sistem manajemen administrasi hewan peliharaan yang profesional, tenang, dan terorganisir.
            </p>
        </div>
    </div>

    <div class="w-full md:w-1/2 flex flex-col justify-center items-center px-6 py-4 sm:px-12 md:px-12 lg:px-16 md:md:min-h-125 pb-[max(1rem,env(safe-area-inset-bottom))]">
        <div class="w-full max-w-md">
            ...
            <div class="mb-4 md:mb-8 text-center md:text-left">
                <h2 class="text-xl sm:text-3xl font-bold text-gray-900 mb-1">Masuk ke Akun Anda</h2>
            ...
            <form action="{{ url('/login') }}" method="POST" class="space-y-3 md:space-y-6">
                @csrf

                <div>
                    <label for="login_id" class="block text-sm font-semibold text-gray-700 mb-1.5">Email atau Username</label>
                    <div class="relative">
        ...
        <div class="mt-4 md:mt-8 w-full max-w-md">
            <div class="relative">
            ...
            <div class="mt-3 md:mt-6 text-center">
                <a href="{{ route('landing.promo') }}" class="w-full flex items-center justify-center gap-2 bg-primary/5 hover:bg-primary/10 text-primary font-bold py-2.5 sm:py-3 px-4 rounded-xl transition duration-300 border border-primary/20">
